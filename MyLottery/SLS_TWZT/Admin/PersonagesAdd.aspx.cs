...
public partial class Admin_PersonagesAdd : AdminPageBase, IRequiresSessionState
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    table = new Tables.T_Personages().Open("ID", "UserName='" + str + "' and LotteryID=" + Utility.FilteSqlInfusion(this.ddlLotteries.SelectedValue), "");
                    if ((table != null) && (table.Rows.Count > 0))
                    {
                        JavaScript.Alert(this.Page, str + "已经是" + this.ddlLotteries.SelectedItem.Text + "的名人了！");
                    }
                    else if (new Tables.T_Personages { Order = { Value = num }, UserName = { Value = str }, LotteryID = { Value = this.ddlLotteries.SelectedValue }, IsShow = { Value = this.cbisShow.Checked } }.Insert() > 0L)
                    {
                        Shove._Web.Cache.ClearCache("Admin_Personages");
                        JavaScript.Alert(this, "添加成功", "Personages.aspx?LotteryID=" + this.ddlLotteries.SelectedValue);
                    }
                    else
                    {
