...
public partial class Admin_SendSMS : AdminPageBase, IRequiresSessionState
{
    ...
    protected void btnSend_Click(object sender, EventArgs e)
    {
        ...
        {
            JavaScript.Alert(this.Page, "请输入短信内容。");
        }
        ...
        else
        {
            ...
            else
            {
                ...
                for (int i = 0; i < aimNames.Length; i++)
                {
                    ...
                    else if (users.Mobile.Trim() == "")
                    {
                        num2++;
                        str = str + "用户 " + aimNames[i] + " 没有输入手机号码！<br />";
                    }
                    else if (PF.SendSMS(base._Site, -1L, users.Mobile, this.tbContent.Text.Trim()) < 0)
                    {
                        num2++;
