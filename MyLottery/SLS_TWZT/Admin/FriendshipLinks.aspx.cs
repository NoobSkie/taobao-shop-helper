...
public partial class Admin_FriendshipLinks : AdminPageBase, IRequiresSessionState
{

    private void BindData()
    {
        DataTable dt = new Tables.T_FriendshipLinks().Open("", "SiteID = " + base._Site.ID.ToString(), "[Order]");
        if (dt == null)
        {
    ...
    {
        base.Response.Redirect("FriendshipLinksAdd.aspx", true);
    }
    ...
    protected void g_ItemCommand(object source, DataGridCommandEventArgs e)
    {
        ...
        {
            base.Response.Redirect("FriendshipLinksEdit.aspx?ID=" + e.Item.Cells[6].Text, true);
        }
        else if (e.CommandName == "Del")
        {
            int returnValue = -1;
            string returnDescription = "";
            if (Procedures.P_FriendshipLinkDelete(base._Site.ID, long.Parse(e.Item.Cells[6].Text), ref returnValue, ref returnDescription) < 0)
            {
                PF.GoError(4, "数据库繁忙，请重试", "Admin_FriendshipLinks");
            }
            else if (returnValue < 0)
            {
                PF.GoError(1, returnDescription, "Admin_FriendshipLinks");
            }
            else
            {
                this.BindData();
