...
public partial class Agent_CardPassword_QueryExpiredCardPassword : CardPasswordPageBase, IRequiresSessionState
{

    private void BindData()
    {
        DataTable cacheAsDataTable = Shove._Web.Cache.GetCacheAsDataTable("CardPassword_QueryExpiredCardPassword_" + base._CardPasswordAgentUser.ID.ToString());
        if (cacheAsDataTable == null)
        {
            ...
            {
                int agentID = -1;
                condition = condition + " and ID = " + new CardPassword().GetCardPasswordID(PF.GetCallCert(), this.tbCardPasswordID.Text.Trim(), ref agentID).ToString();
            }
            ...
            cacheAsDataTable = new Tables.T_CardPasswords().Open("ID, DateTime, Money, Period, State", condition, "");
            if (cacheAsDataTable == null)
            {
                PF.GoError(4, "数据库繁忙，请重试", "CardPassword_QueryExpiredCardPassword");
                return;
            }
            Shove._Web.Cache.SetCache("CardPassword_QueryExpiredCardPassword_" + base._CardPasswordAgentUser.ID.ToString(), cacheAsDataTable);
        }
    ...
    {
        Shove._Web.Cache.ClearCache("CardPassword_QueryExpiredCardPassword_" + base._CardPasswordAgentUser.ID.ToString());
        this.BindData();
    }
    ...
    protected void g_ItemDataBound(object sender, DataGridItemEventArgs e)
    {
        ...
        {
            e.Item.Cells[0].Text = new CardPassword().GenNumber(PF.GetCallCert(), _Convert.StrToInt(base._CardPasswordAgentUser.ID.ToString(), 0), _Convert.StrToLong(e.Item.Cells[3].Text, 0L));
