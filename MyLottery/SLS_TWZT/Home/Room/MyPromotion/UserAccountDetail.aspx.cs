...
public partial class Home_Room_MyPromotion_UserAccountDetail : RoomPageBase, IRequiresSessionState
{
    ...
    private void BindUserAccount()
    {
        ...
        long memberID = _Convert.StrToLong(Utility.GetRequest("ID"), -1L);
        if (memberID < 1L)
        {
        ...
        {
            string key = string.Concat(new object[] { "Home_Room_MyPromotion_UserAccountDetail_", memberID, "_", base._User.ID, "_", fromTime.ToString("yyyyMMdd"), toTime.ToString("yyyyMMdd") });
            ...
            if (cacheAsDataTable == null)
            {
                ...
                DataSet ds = null;
                if (Procedures.P_CpsGetCommendMemberBuyDetail(ref ds, base._Site.ID, base._User.ID, memberID, fromTime, toTime, ref returnValue, ref returnDescription) < 0)
                {
