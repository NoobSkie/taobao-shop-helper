...
public partial class Home_Room_ZCDC_Buy_ZJQS : RoomPageBase, IRequiresSessionState
{
            ...
            try
            {
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    if (num2 == 0)
                    {
                        ...
                        builder.AppendLine("<tr>").AppendLine("<td height=\"24\" colspan=\"16\" align=\"center\" bgcolor=\"#E6E6E6\" class=\"black12\"><strong>").AppendLine(now.ToString("yyyy-MM-dd") + " " + this.getWeekDayName(now.DayOfWeek) + "(10：00--次日10：00)</strong></td>").AppendLine("</tr>");
                    }
                    else if (time3 > time2)
                    {
                        now = now.AddDays(1.0);
                        time2 = time2.AddDays(1.0);
                        builder.AppendLine("<tr>").AppendLine("<td height=\"22\" colspan=\"16\" align=\"center\" bgcolor=\"#E6E6E6\" class=\"black12\"><strong>").AppendLine(now.ToString("yyyy-MM-dd") + " " + this.getWeekDayName(now.DayOfWeek) + "(10：00--次日10：00)</strong></td>").AppendLine("</tr>");
                    }
                    str = ((num2 % 2) == 0) ? "form_tr_2" : "form_tr";
                    if (_Convert.StrToInt(row["IsDue"].ToString(), 0) == 0)
                    {
                        builder.AppendLine("<tr style=\"display:;\" id=\"tr_vs_" + row["NO"].ToString() + "\" index=\"" + num.ToString() + "\" isdisable=\"0\" teamname=\"" + row["LeagueTypeName"].ToString() + "\" class=\"" + str + "\" onmouseout=\"this.className='" + str + "'\" onmouseover=\"this.className='form_tr_3'\">").AppendLine("<td id=\"index_" + num.ToString() + "\" index=\"" + row["NO"].ToString() + "\"><input type=\"checkbox\" checked=\"checked\" onclick=\"sg_vote.cancelSelect(" + num.ToString() + ", this)\" />" + row["NO"].ToString() + "</td>").AppendLine("<td bgcolor=\"" + row["MarkersColor"].ToString() + "\" class=\"bai12_2\"><a href=\"#\" target=\"_blank\">" + row["LeagueTypeName"].ToString() + "</a></td>").AppendLine("<td>" + time3.ToString("HH:mm") + "</td>").AppendLine("<td tradeendtime=\"" + time4.ToString("yyyy-MM-dd HH:mm") + "\" zhggendtime=\"" + time4.ToString("yyyy-MM-dd HH:mm") + "\">" + time4.ToString("HH:mm") + "</td>").AppendLine("<td class=\"blue12_line\"><a href=\"#\" target=\"_blank\">" + row["HostTeam"].ToString() + "</a></td>").AppendLine("<td class=\"blue12_line\"><a href=\"#\" target=\"_blank\">" + row["QuestTeam"].ToString() + "</a></td>").AppendLine("<td><span class=\"red12\">&nbsp;" + row["Result"].ToString() + "&nbsp;</span></td>").AppendLine("<td><input id=\"ck_" + num.ToString() + "_0\" type=\"checkbox\" /><br><span>" + row["ZJQ0SP"].ToString().Replace("--", "") + "</span></td>").AppendLine("<td><input id=\"ck_" + num.ToString() + "_1\" type=\"checkbox\" /><br><span>" + row["ZJQ1SP"].ToString().Replace("--", "") + "</span></td>").AppendLine("<td><input id=\"ck_" + num.ToString() + "_2\" type=\"checkbox\" /><br><span>" + row["ZJQ2SP"].ToString().Replace("--", "") + "</span></td>").AppendLine("<td><input id=\"ck_" + num.ToString() + "_3\" type=\"checkbox\" /><br><span>" + row["ZJQ3SP"].ToString().Replace("--", "") + "</span></td>").AppendLine("<td><input id=\"ck_" + num.ToString() + "_4\" type=\"checkbox\" /><br><span>" + row["ZJQ4SP"].ToString().Replace("--", "") + "</span></td>").AppendLine("<td><input id=\"ck_" + num.ToString() + "_5\" type=\"checkbox\" /><br><span>" + row["ZJQ5SP"].ToString().Replace("--", "") + "</span></td>").AppendLine("<td><input id=\"ck_" + num.ToString() + "_6\" type=\"checkbox\" /><br><span>" + row["ZJQ6SP"].ToString().Replace("--", "") + "</span></td>").AppendLine("<td><input id=\"ck_" + num.ToString() + "_7\" type=\"checkbox\" /><br><span>" + row["ZJQ7SP"].ToString().Replace("--", "") + "</span></td>").AppendLine("<td><input id=\"ck_" + num.ToString() + "\" onclick=\"sg_vote.selectAll(" + num.ToString() + ", this.checked);\" type=\"checkbox\" /></td></tr>");
                        num++;
                    }
                    else
                    {
                        ...
                        int num4 = _Convert.StrToInt(row["ZJQResult"].ToString(), -1);
                        for (int i = 0; i < 8; i++)
                        {
                            if (i == num4)
                            {
                                builder.AppendLine("<td><input type=\"checkbox\" disabled /><br>" + row["ZJQ_SP"].ToString() + "</td>");
                            }
                            else
                            {
