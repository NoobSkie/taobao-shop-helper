...
public partial class TC22X5_22X5_WeiHaoCF : Page, IRequiresSessionState
{
    ...
    private void DrawGridFooter(HtmlTextWriter output, Control ctl)
    {
        ...
        for (int i = 2; i < this.GridView1.Columns.Count; i++)
        {
            ...
            for (int j = 0; j < this.GridView1.Rows.Count; j++)
            {
                ...
                if (((this.GridView1.Rows[j].Cells[i].Text == "2") || (this.GridView1.Rows[j].Cells[i].Text == "3")) || ((this.GridView1.Rows[j].Cells[i].Text == "4") || (this.GridView1.Rows[j].Cells[i].Text == "5")))
                {
                    num3++;
                    this.num[i - 2] = num3;
                    this.sum[i - 2] = (num3 * 50) / num;
        ...
        for (int i = 0; i < this.GridView1.Rows.Count; i++)
        {
            for (int j = 2; j < this.GridView1.Columns.Count; j++)
            {
                ...
                if (this.GridView1.Rows[i].Cells[j].Text == "1")
                {
                    ...
                    this.GridView1.Rows[i].Cells[j].BackColor = Color.FromArgb(0xdf, 0xdf, 0xdf);
                }
    ...
    {
        DataTable table = BLL.TC22X5_WeiHaoCF_SeleteByNum(100);
    ...
    {
        DataTable table = BLL.TC22X5_WeiHaoCF_SeleteByNum(50);
        this.GridView1.DataSource = table;
        this.GridView1.DataBind();
        this.GridViewbindColor();
    }
    ...
    {
        DataTable table = BLL.TC22X5_WeiHaoCF_SeleteByNum(30);
        this.GridView1.DataSource = table;
        this.GridView1.DataBind();
        this.GridViewbindColor();
    }
    ...
    {
        DataTable table = BLL.TC22X5_WeiHaoCF_SeleteByNum(20);
        this.GridView1.DataSource = table;
        this.GridView1.DataBind();
        this.GridViewbindColor();
    }
    ...
    {
        DataTable table = BLL.TC22X5_WeiHaoCF_SeleteByNum(10);
        this.GridView1.DataSource = table;
        this.GridView1.DataBind();
        this.GridViewbindColor();
    }
