...
public partial class Home_TrendCharts_JXSSC_SSC_3X_HZWZST : TrendChartPageBase, IRequiresSessionState
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            table = new TrendChart().SSC_3X_HZWST_Select(0, this.tb1.Text.ToString(), this.tb2.Text.ToString(), ref result);
    ...
    protected void btnTop100_Click(object sender, EventArgs e)
    {
        ...
        table = new TrendChart().SSC_3X_HZWST_Select(100, "", "", ref result);
        if ((table != null) && (table.Rows.Count >= 1))
        {
            int count = table.Rows.Count;
            this.tb1.Text = table.Rows[0]["Isuse"].ToString();
    ...
    protected void btnTop30_Click(object sender, EventArgs e)
    {
        string result = "";
        DataTable table = new DataTable();
        table = new TrendChart().SSC_3X_HZWST_Select(30, "", "", ref result);
        if ((table != null) && (table.Rows.Count >= 1))
        {
            int count = table.Rows.Count;
            this.tb1.Text = table.Rows[0]["Isuse"].ToString();
    ...
    protected void btnTop50_Click(object sender, EventArgs e)
    {
        string result = "";
        DataTable table = new DataTable();
        table = new TrendChart().SSC_3X_HZWST_Select(50, "", "", ref result);
        if ((table != null) && (table.Rows.Count >= 1))
        {
            int count = table.Rows.Count;
            this.tb1.Text = table.Rows[0]["Isuse"].ToString();
    ...
    protected void ColorBind()
    {
        ...
        for (int i = 0; i < this.GridView1.Rows.Count; i++)
        {
            ...
            for (int j = 2; j < 12; j++)
            {
                if ((_Convert.StrToInt(this.GridView1.Rows[i].Cells[j].Text, 0) == 0) && (_Convert.StrToInt(this.GridView1.Rows[i].Cells[j].Text, -1) == 0))
                {
                    int num3 = j - 2;
                    Label label2 = new Label();
                    string[] strArray = new string[9];
                    strArray[0] = "<img src='../Images/red_";
    ...
    protected void DrawGridFooter(HtmlTextWriter output, Control ctl)
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            for (int m = 0; m < 10; m++)
            {
                output.Write("<td height='18px'  onClick=Style(this,'../Images/red_" + m + ".gif') align='center' valign='center' style='cursor:pointer;'>&nbsp;</td>");
            }
            output.Write("<td 　 >&nbsp;</td>");
            ...
            for (int n = 0; n < 10; n++)
            {
                output.Write("<td height='18px'  onClick=Style(this,'../Images/orange_" + n + ".gif') align='center' valign='center' style='cursor:pointer;'>&nbsp;</td>");
            }
        }
