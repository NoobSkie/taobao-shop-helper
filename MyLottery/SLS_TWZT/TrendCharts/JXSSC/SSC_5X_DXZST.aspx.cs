...
public partial class Home_TrendCharts_JXSSC_SSC_5X_DXZST : TrendChartPageBase, IRequiresSessionState
{
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            table = new TrendChart().SSC_5X_DXZST_Select(0, this.tb1.Text.ToString(), this.tb2.Text.ToString(), ref result);
    ...
    protected void btnTop100_Click(object sender, EventArgs e)
    {
        ...
        table = new TrendChart().SSC_5X_DXZST_Select(100, "", "", ref result);
        if ((table != null) && (table.Rows.Count >= 1))
        {
            int count = table.Rows.Count;
            this.tb1.Text = table.Rows[0]["Isuse"].ToString();
    ...
    protected void btnTop30_Click(object sender, EventArgs e)
    {
        string result = "";
        DataTable table = new DataTable();
        table = new TrendChart().SSC_5X_DXZST_Select(30, "", "", ref result);
        if ((table != null) && (table.Rows.Count >= 1))
        {
            int count = table.Rows.Count;
            this.tb1.Text = table.Rows[0]["Isuse"].ToString();
    ...
    protected void btnTop50_Click(object sender, EventArgs e)
    {
        string result = "";
        DataTable table = new DataTable();
        table = new TrendChart().SSC_5X_DXZST_Select(50, "", "", ref result);
        if ((table != null) && (table.Rows.Count >= 1))
        {
            int count = table.Rows.Count;
            this.tb1.Text = table.Rows[0]["Isuse"].ToString();
    ...
    protected void ColorBind()
    {
        for (int i = 0; i < this.GridView1.Rows.Count; i++)
        {
            if (this.GridView1.Rows[i].Cells[3].Text.Length > 1)
            {
                ...
                for (int j = 0; j < this.GridView1.Rows[i].Cells[3].Text.Length; j++)
                {
                    list.Add(this.GridView1.Rows[i].Cells[3].Text.Substring(j, 1));
                }
                list.Sort();
            ...
            if (this.GridView1.Rows[i].Cells[4].Text.Length > 1)
            {
                ...
                for (int m = 0; m < this.GridView1.Rows[i].Cells[4].Text.Length; m++)
                {
                    list2.Add(this.GridView1.Rows[i].Cells[4].Text.Substring(m, 1));
                }
                list2.Sort();
