...
public partial class Lottery_Buy_JX11X5 : RoomPageBase, IRequiresSessionState
{
    public string DZ = "";
    ...
    private string GetIsuseChase(int LotteryID)
    {
        try
        {
            ...
            int num = DataCache.LotteryEndAheadMinute[70];
    ...
    public string GetSchemeBonusScalec()
    {
        ...
        return (str + "|" + str2 + "|" + str3 + "|" + str4 + "|" + str5 + "|11选5");
    }
    ...
    private void GetSYYDJHotAndCool()
    {
        ...
        if (this.lbMiss.Text == "")
        {
            DataTable table = new Tables.T_Isuses().Open("top 100 WinLotteryNumber", "LotteryID = 70 and GETDATE()>EndTime and ISNULL(WinLotteryNumber,'')<>'' and IsOpened=1", "EndTime desc");
            if (table == null)
            {
    ...
    public string SplitScheme(string LotteryNumber, int PlayTypeID)
    {
        Lottery lottery = new Lottery();
        string canonicalNumber = "";
        string[] strArray = lottery[70].ToSingle(LotteryNumber, ref canonicalNumber, PlayTypeID);
        if ((strArray == null) || (strArray.Length < 1))
        {
            return "";
        }
