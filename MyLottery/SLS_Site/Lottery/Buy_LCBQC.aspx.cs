...
public partial class Lottery_Buy_LCBQC : RoomPageBase, IRequiresSessionState
{
    public string DZ = "";
    ...
    {
        string str = "";
        DataTable cacheAsDataTable = Shove._Web.Cache.GetCacheAsDataTable("DataCache_LCBQC");
        if ((cacheAsDataTable == null) || (cacheAsDataTable.Rows.Count == 0))
        {
            cacheAsDataTable = new Tables.T_IsuseForLCBQC().Open(" top 6 * ", "IsuseID = " + IsuseID, "[No]");
            if ((cacheAsDataTable == null) || (cacheAsDataTable.Rows.Count != 6))
            {
                return "";
            }
            Shove._Web.Cache.SetCache("DataCache_LCBQC", cacheAsDataTable, 300);
        }
