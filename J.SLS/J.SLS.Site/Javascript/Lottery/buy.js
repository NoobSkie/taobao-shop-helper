...
function GetSchemeBonusScalec() {
    ...
    if (!isGetSchemeBonusScalec) {
        try {
            ...
            Home_Room_Buy.GetSchemeBonusScalec(currentLotteryID, GetSchemeBonusScalec_callback);

        }
        catch (e) {
            time_GetSchemeBonusScalec = setTimeout("Home_Room_Buy.GetSchemeBonusScalec(" + currentLotteryID + ",GetSchemeBonusScalec_callback);", 2000);
        }
    }
}
...
function GetServerTime(lotteryID) {
    ...
    try {
        ...
        Home_Room_Buy.GetSysTime(GetServerTime_callback);
...
function GetIsuseInfo(lotteryID) {
    ...
    try {
        ...
        Home_Room_Buy.GetIsuseInfo(lotteryID, GetIsuseInfo_callback);
...
function GetIsuseInfo_callback(response) {
    ...
    if (currentLotteryID == 6) {
        ...
        if (($Id("tbPlayTypeName").value == "直选单式" || $Id("tbPlayTypeName").value == "直选复式") && (currentLotteryID == 29 || currentLotteryID == 6)) {
...
function GetNewsInfo(lotteryID) {
    ...
    try {
        ...
        Home_Room_Buy.GetNewsInfo(lotteryID, GetNewsInfo_callback);
...
function GetBindFCExpertList(lotteryID) {
    ...
    try {
        ...
        Home_Room_Buy.GetFCExpertList(lotteryID, GetBindFCExpertList_callback);
...
function GetBindWinNumber(lotteryID) {
    ...
    try {
        ...
        Home_Room_Buy.GetWinNumber(lotteryID, GetBindWinNumber_callback);
...
function BindWinNumber(type) {
    ...
    headHtm += "<div style=\"text-align:right;\"><span id=\"Span1\" class=\"hui12\" ><a href=\"TrendChart.aspx?LotteryID=" + currentLotteryID + "\" target=\"_blank\">更多&gt;&gt;</a></span></div>";
...
function oncbInitiateTypeClick(sender) {
    ...
    if ($Id("CoBuy").checked) {
        ...
        if ($Id("HidLotteryID").value != "1" && $Id("HidLotteryID").value != "2" && $Id("HidLotteryID").value != "15") {
...
function ChangeBackImage(index, lotteryID) {
    ...
    for (var i = 0; i < arr.length; i++) {
        if (index == arr[i]) {
            if (arr[i] == 3 && (lotteryID == 5 || lotteryID == 6 || lotteryID == 39)) {
                table.childNodes[arr[i]].className = "redMenu1";
                if ($Id("imgCYHM") != null) {
                    $Id("imgCYHM").src = 'Images/33.jpg';
                }
            } else {
...
function newCoBuy(lotteryID) {
    ...
    $Id("iframeCoBuy").src = "CoBuy.aspx?Radom=" + Math.random() + "&LotteryID=" + lotteryID + "&IsuseID=" + $Id("HidIsuseID").value;
...
function followScheme(lotteryID) {
    ...
    if ($Id("iframeFollowScheme").src == "") {
        $Id("iframeFollowScheme").src = "FollowScheme.aspx?LotteryID=" + lotteryID;
    }
...
function schemeAll(lotteryID) {
    ...
    $Id("iframeSchemeAll").src = "SchemeAll.aspx?Radom=" + Math.random() + "&LotteryID=" + lotteryID + "&IsuseID=" + $Id("HidIsuseID").value;
...
function PlayTypeIntroduce(lotteryID) {
    ...
    $Id("iframePlayTypeIntroduce").src = "Help/help_" + lotteryID + ".htm";
...
function GetLuckNumber(lotteryID) {
    ...
    var v = Home_Room_Buy.GenerateLuckLotteryNumber(lotteryID, type, name).value;
    if (v.split("|").length < 2) {
        alert(v);
...
function showZXHeight() {
    var lotteryID = document.getElementById("HidLotteryID").value;
...
function btn_OK() {
    ...
    try {
        var LotteryNumber = Home_Room_Buy.AnalyseScheme(document.getElementById("tbLotteryNumbers").value, document.getElementById('HidLotteryID').value, document.getElementById('tbPlayTypeID').value);
...
function isYahoo() {
    ...
    if (location.toLowerCase().indexOf("yahoo.") != -1) {
        btn_OKClicksyahoo();
    }
    else {
        btn_OKClick();
...
function btn_OKClicksyahoo() {
    ...
    if ($Id("Chase").checked) {
        if (StrToInt($Id("LbSumMoney").innerText) > 0) {
            ...
            s_str += "<table cellspacing=\"1\" cellpadding=\"0\" bgcolor=\"#DDDDDD\" width=\"600px\"><tr><td align=\"center\" bgcolor=\"#E9F1F8\">您要申请<label id=\"labSumNum\"></label>投注，详细内容：</td></tr>";
            s_str += "<tr><td align=\"center\" bgcolor=\"#FFFFFF\">总金额： <label id =\"labSumMoney\"></label> 元</td></tr>";
            s_str += "<tr><td align=\"center\" bgcolor=\"#F7FEFA\"><input type=\"checkbox\" id=\"ck_isOk\" onclick=\"showAgreement(this)\" />我已经看过并同意  《<a href=\"javascript:showAgreement1();\" ><label id=\"labLotteryName\"></label>投注协议</a>》</td></tr><tbody id=\"tdpact\" style=\"display:none\"><tr><td colspan=\"3\" style=\"padding-left:20px ;padding-right:20px\" bgcolor=\"#FFFFFF\" ><iframe id=\"Login_Iframes\" name=\"Login_Iframes\" width=\"100%\" frameborder=\"0\"  height=\"150px\"></iframe></td></tr></tbody>";
            s_str += "<tr><td align=\"center\" bgcolor=\"#FFFFFF\" style=\"font-weight:bold;\" align=\"center\"><BR/><input id=\"btnOK\" onclick = \"return btn_OKClick1(this);\" value=\"确定\" type=\"button\" />   &nbsp;&nbsp;&nbsp;   <input id=\"btnNO\" value =\"取消\" type=\"button\" onclick = \"return Cancel();\"/></td></tr></table><input type=\"hidden\" id=\"hdvalues\" value=\"1\" />";
            txt.innerHTML = s_str;
            document.getElementById("msgDiv").appendChild(txt);
            document.getElementById("labSumNum").innerText = LotteryName + $Id("tbPlayTypeName").value;
            document.getElementById("labSumMoney").innerText = $Id("LbSumMoney").innerText;
...
function PageEvent(lotteryID) {
    ...
    if (lotteryID == 5 || lotteryID == 6 || lotteryID == 39) {
...
function CheckLotteryID(lotteryid) { 
    if(lotteryid=='39'||lotteryid=='63'||lotteryid=='64')
    {
        return true;
    }
    else
