...
<div class="Pager">
    <asp:HyperLink ID="hlnkPrev" CssClass="PageSignPrev" runat="server">上一页</asp:HyperLink>
    <asp:Repeater ID="rptPageNumber" runat="server">
        <ItemTemplate>
            <asp:HyperLink ID="hlnkPageNumber" runat="server" CssClass="PageIndex" NavigateUrl='<%# DataBinder.Eval(Container.DataItem, "CurrentUrl") %>'
                Text='<%# DataBinder.Eval(Container.DataItem, "DispalyName") %>'></asp:HyperLink>
        </ItemTemplate>
    </asp:Repeater>
    <asp:HyperLink ID="hlnkNext" CssClass="PageSignNext" runat="server">下一页</asp:HyperLink>
